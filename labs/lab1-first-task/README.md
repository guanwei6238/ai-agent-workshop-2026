...
> **目標**：體感「派任務」跟「問答」的差別，並且**用檢核表驗證它到底做到了什麼**。
...
**用 VS Code 看**，不要在終端機讀 diff：

1. 在 `habit/` 這個資料夾按右鍵 →「以 Code 開啟」（或 `code .`）
2. 左邊側欄點 **原始檔控制**（那個分支圖示，或按 `Ctrl+Shift+G`）
3. **點任何一個有改動的檔案** → 右邊會出現**左右並排**的比對，
   紅色是刪掉的、綠色是新增的

> `init.ts` 已經幫你建好 git repo 並存了初始狀態，所以 VS Code
> 知道「改動前」長什麼樣。**你不用會用 git**，點一點就好。

**要看的是**：它新增了哪個檔？改了 `cli.ts` 的哪幾行？有沒有動到你沒叫它動的東西？
